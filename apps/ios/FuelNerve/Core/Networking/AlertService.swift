...
private struct AlertListResponse: Decodable, Sendable {
    struct Item: Decodable, Sendable {
        ...
        let id: String; let severity: String; let title: String; let message: String
...
struct LiveAlertService: AlertService {
    ...
    func alerts(stationId: String?) async throws -> [OwnerAlert] {
        ...
        return response.alerts.map { item in
            ...
            return OwnerAlert(id: item.id, title: item.title, detail: item.message, severity: severity, evidence: item.evidenceLabel, evidencePath: item.evidencePath, stationName: item.station?.name, createdAt: item.createdAt, readAt: item.readAt, acknowledgedAt: item.acknowledgedAt, packet: item.packet)
        }
    }
    func acknowledge(id: String) async throws {
