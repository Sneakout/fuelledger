...
import { BadgeCheck, BarChart3, BookOpenCheck, Boxes, Building2, ChevronDown, CircleDollarSign, ClipboardList, Fuel, LayoutDashboard, Menu, Package, ReceiptIndianRupee, ShoppingCart, Users, X } from 'lucide-react';
import { useAuth } from './AuthProvider';
import { useStation } from './StationProvider';

const nav = [
  ['Dashboard', '/', LayoutDashboard], ['Petrol Pumps', '/stations', Building2], ['Products', '/products', Package], ['Operations', '/operations', ClipboardList], ['Reconciliation', '/reconciliation', BadgeCheck], ['Inventory', '/inventory', Boxes], ['Sales', '/sales', CircleDollarSign], ['Purchases', '/purchases', ShoppingCart], ['Expenses', '/expenses', ReceiptIndianRupee], ['Customers', '/customers', Users], ['Accounting', '/accounting', BookOpenCheck], ['Reports', '/reports', BarChart3], ['Staff & access', '/staff', Users],
] as const;
export function AppShell() {
  ...
  return <div className="shell">
    <aside className={open ? 'sidebar open' : 'sidebar'} aria-label="Primary navigation">
      ...
      <nav>{nav.map(([label, path, Icon]) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setOpen(false)}><Icon size={19}/><span>{label}</span></NavLink>)}</nav>
