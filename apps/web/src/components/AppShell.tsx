...
import { BadgeCheck, BarChart3, BellRing, BookOpenCheck, Boxes, Building2, ChevronDown, ClipboardList, CreditCard, Fuel, IndianRupee, LayoutDashboard, Lightbulb, Menu, Package, ReceiptIndianRupee, ShoppingCart, Users, X } from 'lucide-react';
...
export function AppShell() {
  ...
  return <div className="shell">
    <aside className={open ? 'sidebar open' : 'sidebar'} aria-label="Primary navigation">
      <div className="brand"><span className="brand-mark"><Fuel size={22}/></span><span>FuelNerve</span><button className="icon-button close" aria-label="Close menu" onClick={() => setOpen(false)}><X/></button></div>
      ...
      <nav>{nav.filter(([label, path])=>(label!=='Subscription'||user?.role==='OWNER')&&(label!=='WhatsApp alerts'||user?.role==='OWNER')&&(path!=='/insights'||user?.role==='OWNER')&&(label!=='Growth desk'||user?.isPlatformAdmin)).map(([label, path, Icon]) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setOpen(false)}><Icon size={19}/><span>{label}</span></NavLink>)}</nav>
