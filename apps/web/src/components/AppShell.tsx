...
export function AppShell() {
  ...
  return <div className="shell">
    <aside className={open ? 'sidebar open' : 'sidebar'} aria-label="Primary navigation">
      ...
      <nav>{nav.filter(([label])=>(label!=='Subscription'||user?.role==='OWNER')&&(label!=='Demo leads'||user?.isPlatformAdmin)).map(([label, path, Icon]) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setOpen(false)}><Icon size={19}/><span>{label}</span></NavLink>)}</nav>
