  ...
  return <div className="invoice-preview-overlay" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}>
    <section ref={dialog} className="invoice-preview-dialog" role="dialog" aria-modal="true" aria-labelledby="invoice-preview-title" tabIndex={-1} onKeyDown={onKeyDown}>
      ...
      <div className="invoice-preview-body">
        ...
        <section className="invoice-preview-section"><header><FileCheck2/><div><h3>Products found</h3><p>These quantities and rates would form the invoice.</p></div></header><div className="invoice-preview-products">{preview.products.map((product, index) => <article key={`${product.description}-${index}`}><span><strong>{product.description}</strong><small>{number(product.quantity)} {product.unit || "units"} at {money(product.unitRate)} each</small></span><strong>{money(product.amount)}</strong></article>)}</div></section>
