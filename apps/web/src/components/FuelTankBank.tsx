...
import './fuel-tank-comparison.css';
...
const recordedToday = (value: string | null, referenceDate: string) => {
  if (!value) return false;
  const reading = new Date(value), today = new Date(referenceDate);
  return reading.getFullYear() === today.getFullYear() && reading.getMonth() === today.getMonth() && reading.getDate() === today.getDate();
};

export function FuelTankBank({ tanks, asOf }: { tanks: Tank[]; asOf: string }) {
  ...
  async function save(tank: Tank) {
    const value = Number(density);
    if (value < 600 || value > 1200) { setError('Enter a density between 600 and 1,200 kg/m³.'); return; }
    setSaving(true);
    setError('');
    try {
      await api.recordDensity({ stationId: tank.station.id, tankId: tank.id, density: value });
  ...
  return <section className="fuel-bank">
    ...
    {tanks.length ? <div className="fuel-bank-grid">{tanks.map(tank => {
      const sequence = (counters[tank.productCode] = (counters[tank.productCode] ?? 0) + 1);
      const colour = tank.productCode === 'MS' ? 'ms' : 'hsd';
      const todayDone = saved[tank.id] !== undefined || recordedToday(tank.densityRecordedAt, asOf);
      const shownDensity = saved[tank.id] ?? tank.density;
      const estimatedStock = tank.expectedFromLastDip ?? tank.bookStock;
      ...
      return <article className={`fuel-orb-card ${colour}${overflow ? ' over-capacity' : ''}`} key={tank.id}>
        <div className="fuel-orb" aria-label={`${tank.productCode} Tank ${sequence}: ${amount(estimatedFill)} percent estimated stock`}>
          ...
          <span className="fuel-percent">{amount(estimatedFill)}%</span>
        </div>
        <div className="fuel-orb-copy">
          <span className="fuel-product">{tank.productCode}</span>
          <h3>{tank.productCode} Tank {sequence}</h3>
          {overflow && <p className="tank-capacity-warning">{estimatedStock > tank.workingCapacity ? 'Shift estimate' : 'Book stock'} exceeds safe capacity. Check readings and tank allocation.</p>}
          <dl>
            <div><dt>Estimated tank stock</dt><dd>{amount(estimatedStock)} L</dd></div>
            <div><dt>Total capacity</dt><dd>{amount(tank.workingCapacity)} L</dd></div>
