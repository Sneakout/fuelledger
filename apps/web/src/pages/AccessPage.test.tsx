...
describe('AccessPage',()=>{it('shows outlet directory and role-based assignments',async()=>{render(<AccessPage/>);expect(await screen.findByRole('heading',{name:'Put the right people at the right outlets'})).toBeInTheDocument();expect(screen.getAllByText('Station C')).toHaveLength(2);expect(screen.getByText('Demo Manager')).toBeInTheDocument();expect(screen.getByText('All fuel stations')).toBeInTheDocument();});});
