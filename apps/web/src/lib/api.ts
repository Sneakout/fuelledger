...
export type ShiftStation = { id:string;name:string;code:string;lastClosing:{shiftId:string;shiftNumber:number;closedAt:string;tankReadings:Array<{id:string;value:string}>;nozzleReadings:Array<{id:string;value:string}>}|null;configurations:Array<{id:string;version:number;tanks:Array<{id:string;code:string;openingStock:string;product:{name:string;code:string}}>;dispensers:Array<{id:string;code:string;nozzles:Array<{id:string;code:string;openingMeter:string;product:{name:string;code:string}}>}>}> };
...
export type CatalogProduct = { id: string; name: string; code: string; hsnCode: string | null; category: string; unit: string; purchasePrice: string; sellingPrice: string; sellingPriceHistory:Array<{id:string;price:string;effectiveFrom:string;createdAt:string}>;inventoryTracked: boolean; tankLinked: boolean; meterLinked: boolean; isService: boolean; active: boolean; taxCategoryId: string | null; customCategoryId: string | null; taxCategory: CatalogTaxCategory | null; customCategory: CatalogCategory | null };
export type ProductForm = Omit<CatalogProduct, 'id' | 'purchasePrice' | 'sellingPrice' | 'sellingPriceHistory' | 'taxCategory' | 'customCategory'> & { purchasePrice: number; sellingPrice: number;sellingPriceEffectiveFrom?:string };
...
export type AccountingPayable={id:string;invoiceNumber:string;invoiceDate:string;dueDate:string;total:number;paid:number;outstanding:number;status:string;overdue:boolean;supplier:{name:string;code:string};station:{name:string;code:string}};
export type AccountingBootstrap={accounts:AccountBalance[];journals:Journal[];payables:AccountingPayable[];trial:{debit:number;credit:number}};
