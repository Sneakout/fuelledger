...
export async function dailyBriefing(organizationId: string, permittedStationIds?: string[], stationId?: string, options: { demoAccess?: boolean } = {}) {
  ...
  for (const action of dashboard.actions.filter(action => action.id.startsWith('price-approval-') || action.id === 'purchases-recorded-today')) add({
    id: action.id,
    category: 'PURCHASES',
    severity: action.severity === 'HIGH' ? 'ATTENTION' : 'INFORMATION',
    label: action.title,
    value: action.id === 'purchases-recorded-today' ? 'Recorded today' : 'Owner confirmation required',
    context: action.detail,
    evidenceLabel: 'Purchase records',
    evidencePath: action.href,
  });
