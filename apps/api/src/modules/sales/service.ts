...
export async function bootstrap(organizationId: string,stationIds?:string[]) {
  const [openShifts, products, employees, sales, customers] = await Promise.all([
    ...
    prisma.product.findMany({ where: { organizationId, active: true }, orderBy: [{ isService: 'asc' }, { name: 'asc' }], select: { id: true, name: true, code: true, unit: true, sellingPrice: true, meterLinked: true, tankLinked: true, isService: true, category: true } }),
  ...
  return { openShifts, products, employees, sales, customers: customers.map(customer => ({ ...customer, outstanding: customer.ledger.reduce((sum, row) => sum + Number(row.amount), 0), ledger: undefined })) };
}
...
async function persistSale({ organizationId, input, kind, quantity, product }: { organizationId: string; input: SaleInput; kind: 'METERED' | 'PRODUCT' | 'SERVICE'; quantity: number; product: { id: string; inventoryTracked: boolean; isService: boolean; purchasePrice: Prisma.Decimal } }) {
  const unitPrice = new Prisma.Decimal(input.unitPrice); const totalAmount = new Prisma.Decimal(quantity).mul(unitPrice);
  return prisma.$transaction(async tx => {
    ...
    if (product.inventoryTracked) await tx.inventoryLedger.create({ data: { organizationId, stationId: input.stationId, productId: product.id, tankId: input.tankId ?? null, type: 'SALE', quantityDelta: new Prisma.Decimal(quantity).neg(), saleId: sale.id, occurredAt: sale.occurredAt, createdById: input.employeeId } });
