...
export async function receivePayment(organizationId: string, customerId: string, createdById: string, input: CustomerReceiptInput) {
  ...
  if (!customer) throw new AppError(404, 'CUSTOMER_NOT_FOUND', 'Choose an active customer account.'); if (!station) throw new AppError(404, 'STATION_NOT_FOUND', 'Choose an active station.');
