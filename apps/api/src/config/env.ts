...
const envSchema = z.object({
  ...
  INVOICE_IMPORT_RELEASE_STAGE: z.enum(["OFF", "LOCAL", "STAGING", "PRODUCTION"]).default("LOCAL"),
  INVOICE_IMPORT_ROLLOUT_PERCENT: z.coerce.number().int().min(0).max(100).default(100),
  INVOICE_IMPORT_MONITORING_PERCENT: z.coerce.number().int().min(0).max(100).default(100),
  INVOICE_IMPORT_ROLLBACK: z.string().default("false").transform(value => value === "true"),
