...
export const managerInputSchema=z.object({name:z.string().trim().min(2).max(100),email:z.email('Enter a valid email address'),temporaryPassword:z.string().min(8,'Temporary password must be at least 8 characters').regex(/[A-Z]/,'Include an uppercase letter').regex(/[0-9]/,'Include a number'),stationId:z.string().cuid()});
export const changePasswordSchema=z.object({password:z.string().min(8,'Password must be at least 8 characters').regex(/[A-Z]/,'Include an uppercase letter').regex(/[0-9]/,'Include a number')});
export type ManagerInput=z.infer<typeof managerInputSchema>;
export type ChangePasswordInput=z.infer<typeof changePasswordSchema>;
export const nozzleCustodySchema=z.object({assignments:z.array(z.object({nozzleId:z.string().cuid(),userId:z.string().cuid()})).min(1)});
...
export const stationSetupSchema = z.object({
  ...
  if (duplicate(setup.products.map(x => x.code))) context.addIssue({ code: 'custom', message: 'Product codes must be unique.', path: ['products'] });
  if (duplicate(setup.tanks.map(x => x.code))) context.addIssue({ code: 'custom', message: 'Tank IDs must be unique.', path: ['tanks'] });
  if (duplicate(setup.dispensers.map(x => x.code))) context.addIssue({ code: 'custom', message: 'Dispenser IDs must be unique.', path: ['dispensers'] });
  const products = new Map(setup.products.map(x => [x.code, x])); const tanks = new Map(setup.tanks.map(x => [x.code, x]));
  for (const [tankIndex, tank] of setup.tanks.entries()) { const product = products.get(tank.productCode); if (!product) context.addIssue({ code: 'custom', message: `Tank ${tank.code} refers to an unknown product.`, path: ['tanks', tankIndex, 'productCode'] }); else if (!product.tankLinked) context.addIssue({ code: 'custom', message: `${product.code} must be tank-linked before assigning it to a tank.`, path: ['tanks', tankIndex, 'productCode'] }); if (tank.workingCapacity > tank.nominalCapacity) context.addIssue({ code: 'custom', message: 'Working capacity cannot exceed nominal capacity.', path: ['tanks', tankIndex, 'workingCapacity'] }); if (tank.openingStock > tank.workingCapacity) context.addIssue({ code: 'custom', message: 'Opening stock cannot exceed working capacity.', path: ['tanks', tankIndex, 'openingStock'] }); }
...
export const productInputSchema = z.object({
  name: z.string().trim().min(2), code: z.string().trim().toUpperCase().regex(/^[A-Z0-9-]+$/), hsnCode: z.string().trim().max(20).optional().or(z.literal('')), category: z.enum(productCategories), customCategoryId: z.string().cuid().nullable().optional(), unit: z.enum(units), purchasePrice: z.coerce.number().min(0), sellingPrice: z.coerce.number().min(0), sellingPriceEffectiveFrom:z.string().datetime().optional(), taxCategoryId: z.string().cuid().nullable().optional(), inventoryTracked: z.boolean(), tankLinked: z.boolean(), meterLinked: z.boolean(), isService: z.boolean(), active: z.boolean(),
}).superRefine((product, context) => { if (product.isService && (product.inventoryTracked || product.tankLinked || product.meterLinked)) context.addIssue({ code: 'custom', message: 'Services cannot be inventory-, tank-, or meter-linked.' }); if ((product.tankLinked || product.meterLinked) && !product.inventoryTracked) context.addIssue({ code: 'custom', message: 'Tank or meter linkage requires inventory tracking.' }); if ((product.tankLinked || product.meterLinked) && !['FUEL','DEF'].includes(product.category)) context.addIssue({ code: 'custom', message: 'Only Fuel and DEF products can be tank- or meter-linked.' }); });
export const categoryInputSchema = z.object({ name: z.string().trim().min(2), code: z.string().trim().toUpperCase().regex(/^[A-Z0-9-]+$/) });
...
export const saleInputSchema = z.object({
  stationId: z.string().cuid(), shiftId: z.string().cuid(), productId: z.string().cuid(), employeeId: z.string().cuid(),
  paymentMethod: z.enum(paymentMethods), unitPrice: z.coerce.number().min(0), quantity: z.coerce.number().positive().optional(),
  tankId: z.string().cuid().nullable().optional(), nozzleId: z.string().cuid().nullable().optional(),
  meterOpening: z.coerce.number().min(0).nullable().optional(), meterClosing: z.coerce.number().min(0).nullable().optional(),
  customerId: z.string().cuid().nullable().optional(), vehicleId: z.string().cuid().nullable().optional(), customerName: z.string().trim().min(2).max(120).nullable().optional(), vehicleNumber: z.string().trim().min(2).max(32).nullable().optional(), notes: z.string().trim().max(500).optional(),
}).superRefine((sale, context) => {
  const metered = sale.meterOpening !== null && sale.meterOpening !== undefined || sale.meterClosing !== null && sale.meterClosing !== undefined;
  if (['CREDIT', 'FLEET'].includes(sale.paymentMethod) && !sale.customerId) context.addIssue({ code: 'custom', message: 'Choose a customer account for credit and fleet sales.', path: ['customerId'] });
  if (metered && (!sale.tankId || !sale.nozzleId || sale.meterOpening === null || sale.meterOpening === undefined || sale.meterClosing === null || sale.meterClosing === undefined)) context.addIssue({ code: 'custom', message: 'Meter sales need a tank, nozzle, opening meter, and closing meter.', path: ['meterClosing'] });
  if (metered && sale.meterClosing! <= sale.meterOpening!) context.addIssue({ code: 'custom', message: 'The closing meter must be greater than the opening meter.', path: ['meterClosing'] });
  if (!metered && !sale.quantity) context.addIssue({ code: 'custom', message: 'Enter a quantity for this sale.', path: ['quantity'] });
});
export type SaleInput = z.infer<typeof saleInputSchema>;

const inventoryLineSchema = z.object({ productId: z.string().cuid(), tankId: z.string().cuid().nullable().optional(), quantity: z.coerce.number().positive(), unitCost: z.coerce.number().min(0) });
export const receiptInputSchema = z.object({ stationId: z.string().cuid(), supplierName: z.string().trim().min(2).max(120), referenceNo: z.string().trim().max(80).optional(), receivedAt: z.string().datetime().optional(), notes: z.string().trim().max(500).optional(), lines: z.array(inventoryLineSchema).min(1) });
export const inventoryAdjustmentSchema = z.object({ stationId: z.string().cuid(), productId: z.string().cuid(), tankId: z.string().cuid().nullable().optional(), quantityDelta: z.coerce.number().refine(value => value !== 0, 'Adjustment cannot be zero.'), notes: z.string().trim().min(3).max(500) });
export const tankReadingInputSchema = z.object({ stationId: z.string().cuid(), tankId: z.string().cuid(), physicalStock: z.coerce.number().min(0), dipReading: z.coerce.number().min(0).nullable().optional(), notes: z.string().trim().max(500).optional() });
export type ReceiptInput = z.infer<typeof receiptInputSchema>; export type InventoryAdjustmentInput = z.infer<typeof inventoryAdjustmentSchema>; export type TankReadingInput = z.infer<typeof tankReadingInputSchema>;

export const reconciliationInputSchema = z.object({
  collections: z.array(z.object({ paymentMethod: z.enum(paymentMethods), actualAmount: z.coerce.number().min(0), adjustmentAmount: z.coerce.number(), adjustmentReason: z.string().trim().max(300).nullable().optional() })).length(paymentMethods.length),
  notes: z.string().trim().max(500).optional(),
}).superRefine((input, context) => {
  const methods = input.collections.map(collection => collection.paymentMethod);
  if (new Set(methods).size !== paymentMethods.length || paymentMethods.some(method => !methods.includes(method))) context.addIssue({ code: 'custom', message: 'Provide one collection row for every payment method.', path: ['collections'] });
  for (const [index, collection] of input.collections.entries()) if (collection.adjustmentAmount !== 0 && (!collection.adjustmentReason || collection.adjustmentReason.length < 3)) context.addIssue({ code: 'custom', message: 'Explain every manual adjustment.', path: ['collections', index, 'adjustmentReason'] });
});
...
export const customerInputSchema = z.object({
  name: z.string().trim().min(2).max(120), code: z.string().trim().toUpperCase().regex(/^[A-Z0-9-]+$/), type: z.enum(customerTypes), phone: z.string().trim().max(30).optional(), email: z.string().trim().email().optional().or(z.literal('')), taxId: z.string().trim().max(30).optional(), billingAddress: z.string().trim().max(300).optional(), creditLimit: z.coerce.number().min(0), creditDays: z.coerce.number().int().min(0).max(365), active: z.boolean(),
});
export const vehicleInputSchema = z.object({ number: z.string().trim().toUpperCase().min(2).max(32), label: z.string().trim().max(80).optional(), active: z.boolean() });
export const customerReceiptInputSchema = z.object({ stationId: z.string().cuid(), amount: z.coerce.number().positive(), paymentMethod: z.enum(['CASH', 'UPI', 'CARD', 'OTHER']), referenceNo: z.string().trim().max(80).optional(), notes: z.string().trim().max(300).optional(), receivedAt: z.string().datetime().optional() });
export type CustomerInput = z.infer<typeof customerInputSchema>; export type VehicleInput = z.infer<typeof vehicleInputSchema>; export type CustomerReceiptInput = z.infer<typeof customerReceiptInputSchema>;

const attachmentSchema=z.object({fileName:z.string().trim().min(1).max(160),mimeType:z.string().trim().min(1).max(100),size:z.coerce.number().int().positive().max(500_000),contentBase64:z.string().min(1).max(700_000)});
const settlementMethods=['CASH','UPI','CARD','OTHER'] as const;
export const supplierInputSchema=z.object({name:z.string().trim().min(2).max(120),code:z.string().trim().toUpperCase().regex(/^[A-Z0-9-]+$/),phone:z.string().trim().max(30).optional(),email:z.string().trim().email().optional().or(z.literal('')),taxId:z.string().trim().max(30).optional(),address:z.string().trim().max(300).optional(),paymentTerms:z.coerce.number().int().min(0).max(365),active:z.boolean()});
export const purchaseInvoiceInputSchema=z.object({stationId:z.string().cuid(),supplierId:z.string().cuid(),invoiceNumber:z.string().trim().min(1).max(80),invoiceDate:z.string().datetime(),dueDate:z.string().datetime(),invoiceTotal:z.coerce.number().positive().optional(),taxAmount:z.coerce.number().min(0).default(0),notes:z.string().trim().max(500).optional(),receiveNow:z.boolean(),paidNow:z.boolean().optional().default(false),paymentMethod:z.enum(settlementMethods).optional(),paymentReferenceNo:z.string().trim().max(80).optional(),attachment:attachmentSchema.nullable().optional(),lines:z.array(z.object({productId:z.string().cuid().nullable().optional(),tankId:z.string().cuid().nullable().optional(),description:z.string().trim().min(2).max(160),quantity:z.coerce.number().positive(),unitCost:z.coerce.number().min(0),taxRate:z.coerce.number().min(0).max(100).default(0),hsnCode:z.string().trim().max(20).optional()})).min(1)}).superRefine((invoice,context)=>{if(new Date(invoice.dueDate)<new Date(invoice.invoiceDate))context.addIssue({code:'custom',message:'Due date cannot be before the invoice date.',path:['dueDate']});if(invoice.receiveNow)for(const [index,line] of invoice.lines.entries())if(!line.productId)context.addIssue({code:'custom',message:'Every received line needs an inventory product.',path:['lines',index,'productId']});if(invoice.paidNow&&!invoice.paymentMethod)context.addIssue({code:'custom',message:'Choose how this invoice was paid.',path:['paymentMethod']});});
export const purchaseInvoiceUpdateSchema=z.object({invoiceNumber:z.string().trim().min(1).max(80),invoiceDate:z.string().datetime(),dueDate:z.string().datetime(),notes:z.string().trim().max(500).optional(),refreshPrices:z.boolean().optional().default(false),markPaid:z.boolean().optional().default(false),paymentMethod:z.enum(settlementMethods).optional(),paymentReferenceNo:z.string().trim().max(80).optional()}).superRefine((invoice,context)=>{if(new Date(invoice.dueDate)<new Date(invoice.invoiceDate))context.addIssue({code:'custom',message:'Due date cannot be before the invoice date.',path:['dueDate']});if(invoice.markPaid&&!invoice.paymentMethod)context.addIssue({code:'custom',message:'Choose how this invoice was paid.',path:['paymentMethod']});});
export const supplierPaymentInputSchema=z.object({stationId:z.string().cuid(),invoiceId:z.string().cuid(),amount:z.coerce.number().positive(),paymentMethod:z.enum(['CASH','UPI','CARD','OTHER']),referenceNo:z.string().trim().max(80).optional()});
export const expenseCategoryInputSchema=z.object({name:z.string().trim().min(2).max(80),code:z.string().trim().toUpperCase().regex(/^[A-Z0-9-]+$/)});
export const expenseInputSchema=z.object({stationId:z.string().cuid(),categoryId:z.string().cuid(),description:z.string().trim().min(3).max(160),amount:z.coerce.number().positive(),paymentMethod:z.enum(settlementMethods),incurredAt:z.string().datetime(),referenceNo:z.string().trim().max(80).optional(),notes:z.string().trim().max(500).optional(),attachment:attachmentSchema.nullable().optional()});
export type SupplierInput=z.infer<typeof supplierInputSchema>;export type PurchaseInvoiceInput=z.infer<typeof purchaseInvoiceInputSchema>;export type PurchaseInvoiceUpdateInput=z.infer<typeof purchaseInvoiceUpdateSchema>;export type SupplierPaymentInput=z.infer<typeof supplierPaymentInputSchema>;export type ExpenseCategoryInput=z.infer<typeof expenseCategoryInputSchema>;export type ExpenseInput=z.infer<typeof expenseInputSchema>;
