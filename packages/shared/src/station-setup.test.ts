...
import { closeShiftSchema, customerInputSchema, customerReceiptInputSchema, customerSubscriptionUpdateSchema, densityReadingInputSchema, expenseCategoryInputSchema, expenseInputSchema, inventoryAdjustmentSchema, openShiftSchema, paymentMethods, productInputSchema, purchaseInvoiceInputSchema, purchaseInvoiceUpdateSchema, receiptInputSchema, reconciliationInputSchema, saleInputSchema, supplierInputSchema, supplierPaymentInputSchema, tankReadingInputSchema, vehicleInputSchema } from './index.js';
...
describe('shift validation', () => {
  ...
  it('accepts an opening shift without fixed nozzle attendants',()=>expect(openShiftSchema.safeParse({stationId:id,managerId:id,userIds:[id],nozzleAssignments:[],openingCash:5000,tankReadings:[{id,value:100}],nozzleReadings:[{id,value:1000}]}).success).toBe(true));
...
describe('shift reconciliation validation', () => {
  ...
  it('accepts documented, fully allocated adjustments',()=>expect(reconciliationInputSchema.safeParse({collections:collections.map(row=>row.paymentMethod==='CASH'?{...row,adjustmentAmount:-10,adjustmentReason:'Bank deposit fee'}:row.paymentMethod==='UPI'?{...row,adjustmentAmount:10,adjustmentReason:'UPI amount classified as cash'}:row)}).success).toBe(true));
...
describe('purchases and expenses validation',()=>{const id='ckshift000000000000000001';const now=new Date().toISOString();
 ...
 it('accepts an inventory-linked purchase invoice',()=>expect(purchaseInvoiceInputSchema.safeParse({stationId:id,supplierId:id,invoiceNumber:'INV-1',invoiceDate:now,receivedAt:now,dueDate:now,taxAmount:18,receiveNow:true,lines:[{productId:id,tankId:id,description:'MS supply',quantity:1000,unitCost:96,taxRate:0}]}).success).toBe(true));
 it('rejects a confirmed invoice total that does not match lines and tax',()=>expect(purchaseInvoiceInputSchema.safeParse({stationId:id,supplierId:id,invoiceNumber:'INV-OCR',invoiceDate:now,dueDate:now,invoiceTotal:120,taxAmount:18,receiveNow:false,lines:[{description:'MS supply',quantity:1,unitCost:96,taxRate:0}]}).success).toBe(false));
 ...
 it('accepts refreshing saved invoice prices',()=>expect(purchaseInvoiceUpdateSchema.safeParse({version:0,invoiceNumber:'INV-1',invoiceDate:now,dueDate:now,refreshPrices:true,correctionReason:'Apply the dated supplier price'}).success).toBe(true));
 it('requires a payment method when marking a saved invoice paid',()=>{const update={version:0,invoiceNumber:'INV-1',invoiceDate:now,dueDate:now,markPaid:true,correctionReason:'Record the confirmed settlement'};expect(purchaseInvoiceUpdateSchema.safeParse(update).success).toBe(false);expect(purchaseInvoiceUpdateSchema.safeParse({...update,paymentMethod:'UPI'}).success).toBe(true);});
 ...
 it('accepts a built-in expense category identifier',()=>expect(expenseInputSchema.safeParse({stationId:id,categoryId:'default_d5e996a7e0c00c0ec43d42320459828b',description:'Electricity bill',amount:2500,paymentMethod:'CASH',incurredAt:now}).success).toBe(true));
 it('keeps supplier and expense settlements out of customer credit accounts',()=>{expect(expenseInputSchema.safeParse({stationId:id,categoryId:id,description:'Electricity bill',amount:2500,paymentMethod:'CREDIT',incurredAt:now}).success).toBe(false);expect(purchaseInvoiceInputSchema.safeParse({stationId:id,supplierId:id,invoiceNumber:'INV-CREDIT',invoiceDate:now,dueDate:now,taxAmount:0,receiveNow:false,paidNow:true,paymentMethod:'CREDIT',lines:[{description:'Supply',quantity:1,unitCost:10,taxRate:0}]}).success).toBe(false);});
  it('accepts every published plan choice and rejects unavailable combinations',()=>{
  const base={paymentConfirmed:true,setupFeePaid:false};
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'MONTHLY'}).success).toBe(true);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'YEARLY'}).success).toBe(true);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'LIFETIME'}).success).toBe(true);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'MONTHLY'}).success).toBe(true);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'YEARLY'}).success).toBe(true);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'FOUNDING_YEARLY'}).success).toBe(true);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'FOUNDING_YEARLY'}).success).toBe(false);
  expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'LIFETIME'}).success).toBe(false);
  });
});
