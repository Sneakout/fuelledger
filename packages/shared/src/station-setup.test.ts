...
describe('purchases and expenses validation',()=>{const id='ckshift000000000000000001';const now=new Date().toISOString();
 ...
 it('requires setup payment before lifetime activation',()=>{expect(customerSubscriptionUpdateSchema.safeParse({setupFeePaid:false,lifetimeAccessPaid:true}).success).toBe(false);expect(customerSubscriptionUpdateSchema.safeParse({setupFeePaid:true,lifetimeAccessPaid:true}).success).toBe(true);});
});
