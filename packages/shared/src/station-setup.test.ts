...
describe('purchases and expenses validation',()=>{const id='ckshift000000000000000001';const now=new Date().toISOString();
 ...
 it('accepts published subscription choices and rejects invalid combinations',()=>{const base={paymentConfirmed:true,setupFeePaid:false};expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'MONTHLY'}).success).toBe(true);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'YEARLY'}).success).toBe(true);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'LIFETIME'}).success).toBe(true);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'MONTHLY'}).success).toBe(true);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'YEARLY'}).success).toBe(true);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'FOUNDING_YEARLY'}).success).toBe(true);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE',billingPeriod:'FOUNDING_YEARLY'}).success).toBe(false);expect(customerSubscriptionUpdateSchema.safeParse({...base,plan:'CORE_INTELLIGENCE',billingPeriod:'LIFETIME'}).success).toBe(false);});
});
